...
sub syntenic_paralogs {
  ...
  CAND: foreach my $pair ( sort {  $synteny{$b}->{'SCORE'} <=> $synteny{$a}->{'SCORE'} } 
      ...
      # We use 2 sources : 
      # orf->sisters() which queries neighbouring ohnologs 
  ...
  ACCEPTOHNO: {
      ...
      $x->ohnolog( $y ); # we set reciprocals automatically	    
      map { $seen{ $_->sn }++ } ($x,$y);      
...
sub remember {
	...
	foreach my $o ( $c->stream ) {
	    ...
	    $o->remember( 'name' => $self->name ); # can extract Chr
	    $o->remember( 'coords' => [$self->coords] );
	    $o->remember( 'time' => $TIME );
	}
    }
