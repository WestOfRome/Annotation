...
sub homolog_DEPRACATED {
    my $self = shift;
    my $args = {@_};
    
    $args->{'-synteny'} = 0 unless exists $args->{'-synteny'};
    $args->{'-evalue'} = 1e-5 unless exists $args->{'-evalue'};
    $args->{'-species'} = [ grep {!/YGOB/} (keys %HOMOLOGY,'GENE') ] unless exists $args->{'-species'};
    ...
    my %count;
    foreach my $key ( @{$args->{'-species'}} ) {
	next unless $self->data($key) && $self->data('_'.$key) <= $args->{'-evalue'};
	next unless $self->querysynteny(-restrict => [$key]) >= $args->{'-synteny'};
	$count{ $key }=$self->data('_'.$key) and next unless $args->{'-object'};
	# second gene ? 
	next unless $orf->data($key) && $orf->data('_'.$key) <= $args->{'-evalue'};
	next unless $orf->querysynteny(-restrict => [$key]) >= $args->{'-synteny'};
	$count{ $key }=$self->data('_'.$key);
    }
    return () unless keys %count;

    # 

    my ($species,@other) = sort { $count{$a} <=> $count{$b} } keys %count;
    $species = 'SGD' if $count{$species} == $count{'SGD'};
    my $homolog = $self->data( $species );
    my ($aafile) = $self->fetch(-id => $homolog, -file => 1, -molecule => 'aa' );
    return () unless -e $aafile && -s $aafile > 20;
...
sub reference_DEPRACATED {
    my $self=shift;
    my $ref = shift;
    $self->throw("Depracated [$ref]: Use homolog() ");

    my $file;
   if ( $self->isa(ref($ref)) ) {
	$file = $ref->_write_temp_seq;
    } elsif ( exists $HOMOLOGY{uc($ref)} || uc($ref) eq 'GENE' ) {
        $file = '/Users/devin/Code/perl/Anna/db/single/'.$self->data($ref).'.fsa';
	if ( ! -e $file ) {
	    map { $ref = $_ if defined $self->data( $_ ) && 
		      $self->data( '_'.$_ ) < $self->data( '_'.$ref ) } keys %HOMOLOGY;
	    $file = '/Users/devin/Code/perl/Anna/db/single/'.$self->data($ref).'.fsa';
	}
    } elsif ( -e $ref ) { 
	$file = $ref; 
    } else { $file = '/Users/devin/Code/perl/Anna/db/single/'.$ref.'.fsa'; }
    $self->throw("$ref, $file") unless -e $file;    
...
#=head2 orthogroup_NA
#Version 3. Now only 4 references (from 12 and 8) and no new objects 
#but still causes Storable to segfault. The deep circular referencing 
#seems to be the problem. With 3 genomes all three schemes work and 
#adition of circular references is not a problem. With 4 none of the
#schemes work. It can be shown that it is not a reference count limit
#by adding non-circular references. 
#The non-circular implementation requires designation of a reference 
#genome and only 3 references (for 4 species). 
#=cut 

sub orthogroup_DEPRACATED {
    my $self = shift;
    my $og = shift;

    $self->throw("Orthogroups Not Available.");

    if ( $og =~ /\:\:/ ) { # set 
	
	if ( ! $self->orthogroup && ! $og->orthogroup ) {
	    $self->{ORTHOGROUP} = $og;
	    $og->{ORTHOGROUP} = $self;
	} elsif ( $self->orthogroup && $og->orthogroup ) {
	    my $t1 = $self->orthogroup;	
	    my $t2 = $og->orthogroup;
	    $self->{ORTHOGROUP} = $t2;
	    $og->{ORTHOGROUP} = $t1;	    
	} else {
	    my ($major, $minor) = sort { $b->orthogroup cmp $a->orthogroup } ($self, $og);
	    my $target = $major->orthogroup;
	    $major->{ORTHOGROUP} = $minor;
	    $minor->{ORTHOGROUP} = $target;    
	}

    } elsif ( length($og) == 4 ) { # get 
	my $orf;
	my $x = $self->orthogroup;
	until ( ! $x || $x eq $self || $x->up->up->organism =~ /$og/i ) {
	    $x = $self->orthogroup;
	}
	return ($x eq $self ? undef : $x);
